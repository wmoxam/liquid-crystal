...
class IncludeTagTest < Minitest::Test
  ...
  def test_include_tag_with
    assert_equal "Product: Draft 151cm ",
      Template.parse("{% include 'product' with products[0] %}").render( _h({"products" => [ {"title" => "Draft 151cm"}, {"title" => "Element 155cm"} ]})  )
  end

  def test_include_tag_with_default_name
    assert_equal "Product: Draft 151cm ",
      Template.parse("{% include 'product' %}").render( _h({"product" => {"title" => "Draft 151cm"}})  )
  end

  def test_include_tag_for
    assert_equal "Product: Draft 151cm Product: Element 155cm ",
      Template.parse("{% include 'product' for products %}").render( _h({"products" => [ {"title" => "Draft 151cm"}, {"title" => "Element 155cm"} ]})  )
  end
  ...
  def test_include_tag_with_multiple_local_variables_from_context
    assert_equal "Locale: test123 test321",
      Template.parse("{% include 'locale_variables' echo1: echo1, echo2: more_echos.echo2 %}").render(_h({"echo1" => "test123", "more_echos" => { "echo2" => "test321"}}))
  end
  ...
  def test_nested_include_with_variable
    ...
    assert_equal "Product: Draft 151cm details ",
      Template.parse("{% include 'nested_product_template' with product %}").render(_h({"product" => {"title" => "Draft 151cm"}}))

    assert_equal "Product: Draft 151cm details Product: Element 155cm details ",
      Template.parse("{% include 'nested_product_template' for products %}").render(_h({"products" => [{"title" => "Draft 151cm"}, {"title" => "Element 155cm"}]}))
  ...
  def test_dynamically_choosen_template
    ...
    assert_equal "Test123", Template.parse("{% include template %}").render(_h({"template" => "Test123"}))
    assert_equal "Test321", Template.parse("{% include template %}").render(_h({"template" => "Test321"}))

    assert_equal "Product: Draft 151cm ", Template.parse("{% include template for product %}").render(_h({"template" => "product", "product" => { "title" => "Draft 151cm"}}))
  end
end # IncludeTagTest
