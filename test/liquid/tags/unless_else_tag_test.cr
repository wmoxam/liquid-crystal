...
class UnlessElseTagTest < Minitest::Test
  ...
  def test_unless_in_loop
    assert_template_result "23", "{% for i in choices %}{% unless i %}{{ forloop.index }}{% endunless %}{% endfor %}", _h({"choices" => [1, nil, false]})
  end

  def test_unless_else_in_loop
    assert_template_result " TRUE  2  3 ", "{% for i in choices %}{% unless i %} {{ forloop.index }} {% else %} TRUE {% endunless %}{% endfor %}", _h({"choices" => [1, nil, false]})
  end
end # UnlessElseTest
